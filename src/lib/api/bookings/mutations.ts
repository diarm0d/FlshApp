...
export const createBooking = async (booking: NewBookingParams) => {
  ...
  const newBooking = insertBookingSchema.parse({
    ...booking,
...
export const updateBooking = async (
  id: BookingId,
  booking: UpdateBookingParams
) => {
  const { session } = await getUserAuth();
  const { id: bookingId } = bookingIdSchema.parse({ id });
  const newBooking = updateBookingSchema.parse({
    ...booking,
    userId: session?.user.id!,
  });
  try {
...
export async function createBookingAction(formData: FormData) {
  ...
  const customTitle = `${formData.get("name")} | ${formData.get("flashTitle")}`;
